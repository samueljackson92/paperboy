"""Generic journal RSS/Atom feed source."""
...
class JournalSource(PaperSource):
    ...
    Fully driven by config: supply a name, feed URL, and optional keyword
    filter list. No subclassing needed for new journals.
    ...
    async def fetch_latest(self, limit: int = 50) -> list[Paper]:
        ...
        papers = self._parse(xml_text, limit)
        return [p for p in papers if self._matches_keywords(p)]
