...
SOURCE_STYLES: dict[SourceKind, str] = {
    SourceKind.ARXIV: "arxiv",
    SourceKind.OPENREVIEW: "openreview",
    SourceKind.JOURNAL: "journal",
}

UNREAD_INDICATOR = "●"  # filled circle
READ_INDICATOR = " "
...
class PaperList(Widget):
    ...
    def action_select_paper(self) -> None:
        ...
        if table.cursor_row < len(self._visible_papers):
            paper = self._visible_papers[table.cursor_row]
            self.post_message(self.PaperSelected(paper))

    def action_toggle_read(self) -> None:
        table = self.query_one(DataTable)
        if table.cursor_row < len(self._visible_papers):
            paper = self._visible_papers[table.cursor_row]
            self.post_message(self.PaperToggled(paper))

    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        if event.cursor_row < len(self._visible_papers):
            paper = self._visible_papers[event.cursor_row]
            self.post_message(self.PaperSelected(paper))

    def update_paper(self, updated: Paper) -> None:
        """Replace one paper in-place and re-render."""
        self._all_papers = [
            updated if p.id == updated.id else p
