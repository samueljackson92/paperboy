...
class OpenReviewSource(PaperSource):
    ...
    def __init__(self, venue_id: str, client: Any | None = None) -> None:
    ...
    def _note_to_paper(self, note: Any) -> Paper:
        ...
        if isinstance(authors_raw, dict):
            authors = list(authors_raw.get("value", []))
        else:
        ...
        if isinstance(keywords_raw, dict):
            categories = list(keywords_raw.get("value", []))
        else:
            categories = list(keywords_raw) if keywords_raw else []
